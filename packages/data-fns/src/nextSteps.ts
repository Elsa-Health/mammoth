...
export function basic<
	C extends Condition,
	M extends _AllMedications,
	L extends string
>(conditionsFn: () => C[], medicationsFn: () => M[], labTestsFn: () => L[]) {
	...
	Object.entries(nextStepsBasic)
		...
		.map((s) => {
			...
			d["medications"] = s["medications"]
				.filter((f) => meds.includes(f.id as any))
				.map(
					(f) => ({ id: f.id, text: f.text } as NextSteps.MoreInfo<M>)
				);
			d["testRecommendations"] = s["test-recommendations"]
				.filter((f) => labTests.includes(f.id as any))
				.map(
					(f) => ({ id: f.id, text: f.text } as NextSteps.MoreInfo<L>)
				);
...
>(conditionsFn: () => C[], medicationsFn: () => M[], labTestsFn: () => L[]) {
	...
	const _localeVals = Object.entries(nextStepsExtended)
		...
		.map((s) => {
			...
			// @ts-ignore
			d["medications"] = s["medications"].filter((t) =>
				meds.includes(t.id as any)
			);

			// @ts-ignore
			d["testRecommendations"] = s["test-recommendations"].filter((t) =>
				labTests.includes(t.id as any)
			);
	...
	return {
		...
		locale: (lang: Langugage) => {
			const _vals = {} as {
				[id in C]: NextSteps.Basic<M, L> & NextSteps.ExtendedVals<M, L>;
			};
			_localeVals
				.map((lv) => {
					const nlv = { id: lv.id } as {
						id: C;
					} & NextSteps.ExtendedVals<M, L>;
					nlv["referAndTriageLevel"] =
						lv["referAndTriageLevel"][lang];
					nlv["medications"] = lv["medications"]
						.map((s) => s[lang])
						.filter((s) => s !== undefined) as M[];
					nlv["testRecommendations"] = lv["testRecommendations"]
						.map((s) => s[lang])
						.filter((s) => s !== undefined) as L[];
					return nlv;
				})
				.forEach((s) => {
					const { id, ...other } = s;
