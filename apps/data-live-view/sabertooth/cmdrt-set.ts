...
export class SBSet<Op, T> extends Set<SBState<Op, T>> {
  private _states: Set<SBState<Op, T>>;

  constructor(arr?: SBState<Op, T>[]) {
    super();
    // all states
    this._states = new Set<SBState<Op, T>>(arr || []);
    return this;
  }
  ...
  states() {
    ...
    return new Set<State<Op, T>>(
      Array.from(this._states.values()).map(s => s.state),
    );
  }
