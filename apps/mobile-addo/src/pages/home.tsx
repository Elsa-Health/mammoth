...
function MinifiedPatientHistory () {
    ...
    const REMAINING_MARKS = React.useMemo(() => Math.max(0, data.length - TOTAL_COUNT_TO_SHOW), [data])
    ...
    return (
        <Fragment>
            ...
            <Heading style={{ fontSize: 35  }} font="bold">{data.length}</Heading>
            <View style={{ marginVertical: 6 }}>
                {
                    ...
                    }).slice(0, TOTAL_COUNT_TO_SHOW).map((d, ix) => {
                        const { assessmentInfo: ai, diagnosis } = d.record

                        return (
                            <View key={`visitation-${ix}`} style={{ borderTopColor: theme.color.secondary.light, marginTop: 8, paddingTop: 8,  borderTopWidth: ix !== 0 ? 1: 0 }}>
                                <Text>{ai.sex === 'male' ? tc`sex.male` : tc`sex.female` }, {properAgeString(ai.age)}</Text>
                                ...
                                <Text font="medium" style={{ textTransform: 'capitalize', marginTop: 4, fontSize: 12, color: "#777" }}>{tc`date`}: {dayjs(d.record.dateTime).format("YYYY, Do MMM HH:mm:ss")}</Text>
                            </View>
                        )
                    })
                }
            </View>
            {
                REMAINING_MARKS !== 0 && (
                    <View style={{ paddingVertical: 4 }}>
                        <Text font="medium">{t('subtext.present', { count: REMAINING_MARKS })}</Text>
                    </View>
                )
            }
        </Fragment>
        ...
        <Layout hideHeader>
            <View style={{ display: 'flex', width: '100%', justifyContent: 'center', marginVertical: 16  }}>
                <View style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row' }}>
                    ...
                    >
                        <SettingsIcon style={{ margin: 6 }} /><Text font="medium">{t`settings.title`}</Text>
                    </Pressable>
                </View>
                <View style={{ marginTop: 12 }}>
                    <Text font="extra-black" style={{ fontSize: 26 }}>{t`home.greetings.hi`},</Text>
                    <Text font="extra-black" style={{ fontSize: 26, marginTop: 6, }}>{user?.name}</Text>
                </View>
            </View>
            ...
            <ScrollView style={{}}>
                ...
                <View style={{ display: 'flex', flexDirection: 'column', marginVertical: 10}}>
                    ...
                    <Text style={{ marginVertical: 5 }}>{t`home.new_assessment.description`}</Text>
                    <View style={{ marginTop: 10, width: '100%' }}>
                        <Pressable  
                            ...
                            style={[buttonStyles.primaryButton, { borderWidth: 0, borderRadius: 100, paddingHorizontal: 16, borderColor: theme.color.primary.dark }, { justifyContent: 'center' }]}
                        >
                            <PlusIcon style={{ marginRight: 8, marginVertical: 4, color: "#FFF" }} /><Text font="bold" style={{ color: "#FFF" }}>{t`home.new_assessment.action`}</Text>
                        </Pressable>
                    </View>
                </View>
                <View style={{ flex: 1 }}>
                    ...
                    <View style={{ marginVertical: 16 }}>
                        ...
                        <View style={{ marginTop: 10, width: '100%' }}>
                            <Pressable
                                ...
                                style={[buttonStyles.primaryOutlineButton, { paddingHorizontal: 0 }, { justifyContent: 'center' }]}
                            >
                                <ArchiveIcon style={{ marginRight: 8, marginVertical: 4, color: theme.color.primary.dark }} /><Text font="bold" style={{ color: theme.color.primary.dark }}>{t`home.history.action`}</Text>
                            </Pressable>
                        </View>
                    </View>
                </View>
