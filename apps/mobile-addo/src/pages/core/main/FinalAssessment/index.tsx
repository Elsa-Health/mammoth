...
const nse = data.nextSteps.extended(data.conditions.ids, data.medications.all.ids, data.labTests.ids)
const nextSteps = nse.all
function RecommendedInfo ({ conditionId, collapsed = false }: { conditionId: data.Condition, collapsed?: boolean }) {
    ...
    const { t } = useTranslation('translation', { keyPrefix: 'assessment.feedback.next_steps'})

    React.useEffect(() => {
        console.debug("Working on a collapse function.")
    }, [])

    const testRecommendations = React.useMemo(() => s.testRecommendations
                                        .map(m => ({ id: m.id, text: m[lang]}))
                                        .filter(s => s.text !== undefined), [lang, s])

    const medications = React.useMemo(() => s.medications
                                        .map(m => ({ id: m.id, text: m[lang]}))
                                        .filter(s => s.text !== undefined), [lang, s])
    ...
    return (
        ...
        >
            <Text font="bold" style={{ fontSize: 20, marginVertical: 5 }}>{ data.conditions.name.fromId(conditionId) }</Text>

            {/* information if you are refering the information */}
            <RevealContent style={{ marginVertical: 5 }}  show={s.referAndTriageLevel?.[lang] !== undefined}>
                <View  style={{ display: 'flex', flexDirection: 'row' }}>
                    <View style={{ width: 5, height: 5, backgroundColor: theme.color.secondary.base, borderRadius: 8, margin: 8 }} />
            ...
            <RevealContent  style={{ marginVertical: 5 }}  show={medications.length > 0}>
                <View  style={{ display: 'flex', flexDirection: 'row' }}>
                    <View style={{ width: 5, height: 5, backgroundColor: theme.color.secondary.base, borderRadius: 8,  margin: 8 }} />
                    <View>
                        <Text font="medium" style={{ marginVertical: 5 }}>{t`dispense_meds`}</Text>
                        {
                            medications
                                .map((m, ix) => (
                                    <View key={m.id} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start' }}>
                                        <View><Text>{ix + 1}.</Text></View>
            ...
            <RevealContent style={{ marginVertical: 5 }}  show={testRecommendations.length > 0}>
                <View  style={{ display: 'flex', flexDirection: 'row' }}>
                    <View style={{ width: 5, height: 5, backgroundColor: theme.color.secondary.base, borderRadius: 8,  margin: 8 }} />

                    <View>
                        <Text font="medium" style={{ marginVertical: 5 }}>{t`recommend_tests`}</Text>
                        {
            ...
            <RevealContent style={{ marginVertical: 5 }} show={s.otherRecommendations?.[lang]?.trim() !== undefined}>
                <View style={{ display: 'flex', flexDirection: 'row' }}>
                    <View style={{ width: 5, height: 5, backgroundColor: theme.color.secondary.base, borderRadius: 8,  margin: 8 }} />
...
export default function FinalAssessmentScreen ({ route }: { 
    route?: { params?: { selected_conditions: { data: Differential }[], elsa_conditions: Differential[] } }
}) {
    const navigation = useNavigation()
    const [selectedConditions, setSelectedConditions] = React.useState<ConditionId[]>([])
    ...
    const searchableConditions = React.useMemo(() => {
        ...
        const selected_children = selected_conditions.map(s => getCondition(s.data.condition))
            ...
            ...(
                selected_children.length > 0 ? (
                    [{
                        ...
                        id: 0,
                        children: selected_children.map(d => ({ name: d?.condition, id: d?.id}))
                    }]
                ): 
                []
            ),
    ...
    React.useEffect(() => {
        ...
        .map(
            symp => presentingSymptoms
                    .includes(symp as SymptomId)
        ).reduce((x, y) => x || y)


        // basic condtion is met and ors is not supplied
        setShouldGiveORS(meetsBasicCondition && !medications.includes('oral-rehydration-salts-ors'))
    }, [vals, assessmentInfo])
    ...
    const addORSMedication = React.useCallback(() => {
        if (shouldGiveORS) {
             ...
             */
            setVals(s => produce(s, df => { 
                df.dispensed_medication.selected = true
                df.dispensed_medication.medications.push('oral-rehydration-salts-ors')
                return df
            }))
        }
    ...
    const confirmAssessment = React.useCallback(() => {
        ...
        const useChoices = selectedConditions.map(getCondition).filter(s => s !== undefined) as { condition: string, id: string}[]
                            

        // console.log({ useChoices })
        if (selectedConditions.length > 0) {
            addAssessment({
                id: 'ET-' + Math.ceil((Math.random() * 100) + 1).toString() + '-' + (new Date()).getTime().toString(),
                assessmentInfo,
                diagnosis: {
                    elsa: elsa_conditions,
                    user: useChoices.map(s => ({ condition: s.id.toString(), label: s.condition }))
                }
            }, vals)

            // navigate (by replacement to the final screen)
            navigation.dispatch(
                StackActions.replace('app.history')
            )
            return;
        }
        setErr(t`err.text_make_selection`)
    }, [assessmentInfo, selectedConditions, addAssessment, navigation, route, vals, t])

    return (
        <Layout navigation={navigation} title={t`title`} style={{ padding: 0, paddingTop: 0 }}>
            <ScrollView  style={{ flex: 1, paddingHorizontal: 24, paddingVertical: 8,  }}>
                <View style={{ flex: 1 }}>
                    {/* Condition Decision */}
                    <View style={{ borderBottomColor: theme.color.secondary.light, marginBottom: 10, borderBottomWidth: 1, paddingBottom: 6 }}>
                        <Text font="bold" style={{ marginVertical: 10, fontSize: 18 }}>{t`condition_decision.title`}</Text>
                    ...
                    <RevealContent show={selectedConditions.length > 0} style={{ marginVertical: 10, }}>
                        <View>
                            <Text font="bold" style={{ marginVertical: 10, fontSize: 18 }}>{t`next_steps.title`}</Text>
                    ...
                    <View>
                        <Text font="bold" style={{ marginVertical: 10, fontSize: 18 }}>{t`recommendations.title`}</Text>
                        ...
                        <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <CheckBox 
                                ...
                                onValueChange={newValue => setVals(s => ({ ...s, ref_nearest: newValue }))}
                            />
                        </View>
                        ...
                        <View style={{ marginVertical: 12 }}>
                            ...
                            <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                ...
                                <CheckBox 
                                    ...
                                    onValueChange={checked => setVals( s => produce(s, df => {
                            ...
                            <RevealContent  show={vals.refered_lab_testing.selected}>
                                <SectionedSelect
                                    ...
                                    onSelectedItemsChange={testIds => setVals(s => ({ ...s, refered_lab_testing: { ...s.refered_lab_testing, tests: testIds }}))}
                        ...
                        <View style={{ marginVertical: 12 }}>
                                ...
                                shouldGiveORS && (
                                    ...
                                    >
                                        <View style={{ flex: 1, display: 'flex', flexDirection:'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <ExclamationIcon width={28} height={28} style={{ color: "#ff960d" }} />
                                            <View style={{ marginLeft: 5 }}>
                                                <Text font="bold" style={{ color: "#ff960d", lineHeight: 20 }}>{t`next_steps.supply_ors_text`}</Text>
                                            </View>
                                        </View>
                                        <Pressable onPress={addORSMedication} style={{ borderRadius: 100, marginTop: 10, backgroundColor: "#ff960d", paddingVertical: 5, paddingHorizontal: 16  }}> 
                                            <Text font="bold" style={{ color: "#fff" }}>{t`next_steps.supply_ors_button`}</Text>
                                        </Pressable>
                                    </View>
                                )
                            }
                            <RevealContent show={vals.dispensed_medication.selected}>
                                <SectionedSelect
                                    ...
                                    onSelectedItemsChange={medicationIds => setVals(s => ({ ...s, dispensed_medication: { ...s.dispensed_medication, medications: medicationIds }}))}
                        ...
                        <View style={{ marginVertical: 24 }}>
                            ...
                            <TextInput 
                                ...
                                onChangeText={text=> setVals(s => ({ ...s, recommendations: text }))} 
                            />
                        </View>
                    </View>
                </View>
            </ScrollView>
            <RevealContent style={{ borderColor: theme.color.primary.dark, borderTopWidth: 1 }} show={selectedConditions.length > 0}>
                <View style={{ flex: 1 }} />
                <View style={{ display: 'flex', flexDirection: 'row', justifyContent: "flex-end"}}>
                    <View style={{ marginVertical: 16, paddingHorizontal: 16 }}>
                        <Pressable
                            onPress={confirmAssessment}
                            style={{padding: 6}}
                        >
                            <Text font="bold" style={{ paddingHorizontal: 8 }}>{tc`complete`}</Text>
                            <CheckIcon />
                        </Pressable>
                    </View>
                </View>
