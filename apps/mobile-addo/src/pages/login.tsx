...
export default function Login () {
    ...
    const onSuccess = React.useCallback((e: BarCodeReadEvent) => {
        authenticate(e.data)
            .then(info => {
                login({
                    name: _.upperFirst(info.firstName) + " " + _.upperFirst(info.lastName),
                    uid: info.id,
                })
            })
            .catch(err => {
    ...
    return (
        <Layout title={t`title`} style={{ padding: 0 }}>
            <QRCodeScanner
                ...
                showMarker
                customMarker={
                    <View style={{  borderColor: theme.color.secondary.light, borderWidth: 1, width: 250, height: 250, borderRadius: 5  }} />
                }
                topContent={
