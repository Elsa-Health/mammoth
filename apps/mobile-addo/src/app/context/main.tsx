...
import { Age, AssessmentRecord, SymptomRecord, _CompleteSymptomData } from "../../../@types";
...
type StateAssessmentRecord = { record: AssessmentRecord, recommendations: Partial<Recommendations> }
interface MainState {
    /**
     ...
     */
    addAssessmentRecord: (ar: Omit<AssessmentRecord, 'dateTime'>, recommendations?: Partial<Recommendations>) => void
}


const uploadAsessment = (assessment: StateAssessmentRecord, userId: string) => {
    const { record: { assessmentInfo, id }, recommendations } = assessment                
    ...
    aStatsColl
        .doc(userId)
        .set({
            isTest: ['kevin-james', 'c5d2e724-2e27-11eb-adc1-0242ac120002', 'ba1d95ac-5977-11eb-ae93-0242ac130002'].includes(userId),
...
const createStore = () => create<MainState>(
    persist(
        (set, get) => ({
            assessments: [],
            addAssessmentRecord: (ar, recommendations = {}) => {
                set(s => produce(s, df => {
                    df.assessments.push({ record: { ...ar, dateTime: new Date() }, recommendations })
                    return df
                }))
            },
        }),
...
function getAgeGroup(months: number) {
    ...
    if (months < 72) { return '6 - 72 months' }
    if (months < 120) { return '72 - 120 months' }
    if (months < 228) { return '120 - 228 months' }
    if (months >= 228) { return '228+ months' }
    return '(-inf, inf+)'
}

function getSypmtomItem (_s: SymptomRecord & { data: Partial<_CompleteSymptomData> } | any) : string | undefined {
...
function getAssessmentInfo (stateRecord: StateAssessmentRecord) {
    const { record: { dateTime, assessmentInfo: { age, sex, pregnant, presentingSymptoms, absentSymptoms  }, id, diagnosis: { user, elsa } }, recommendations } = stateRecord  
    ...
    return {
        ...
        symptoms: { present: presentingSymptoms.map(getSypmtomItem), absent: absentSymptoms.map(getSypmtomItem) },
        diagnosis: {
