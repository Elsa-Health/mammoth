...
import { Assessment, Differential, Symptom, SymptomData, SymptomRecord } from '../../../@types'
...
interface AssessmentState extends Assessment {
    ...
    setSymptom: (symptom: SymptomRecord, present?: boolean, data?: SymptomData) => void
...
const builderStoreCreator = (intialState: Pick<AssessmentState, 'age' | 'sex' | 'pregnant'>) => () => create<AssessmentState>((set, get) => ({
    recommended: [],
    presentingSymptoms: [],
    absentSymptoms: [],
    ...
    removeSymptomFromId: (symptomId: string) => {
        set(s => produce(s, df => {
            const presentIndex = df.presentingSymptoms.findIndex(ps => ps.id === symptomId)
            const absentIndex = df.absentSymptoms.findIndex(as => as.id === symptomId)
 ...
 */
function SymptomAssessmentSequenceProvider (props: { children: React.ReactNode, initialState: Pick<AssessmentState, 'age' | 'sex' | 'pregnant'> }) {
    return (
...
export async function fetchFromElsaLambda (patient: any, psstr: Array<SymptomRecord & { data: SymptomData }>, asstr: string[]) {        
    const data = {
        symptoms: {
            present: [...psstr].map(s =>
                _.omit(donparTransform(s.id, s.data), []) // ['Duration']),
            ),
            absent: asstr
        },
        ...patient,
 ...
 * @returns 
 */
export const bottleCapDiscretization = (sortedConditions: Differential[], asstr: string[], topn: number, rangeToFit: number) => {
    ...
    const firstRatio = Math.min(1, (first.data.p * toleranceValue) / Math.max(1, (sum * toleranceValue)))
    const firstCaps = Math.round(firstRatio * rangeToFit)
    const remainingCaps = rangeToFit - firstCaps

    const lastsCountsRatio = lsConditions.map(d => ( ((d.data.p * toleranceValue) / Math.max(1, sum * toleranceValue)) ))
 ...
 * @param rangeToFit 
 */
export const normalValueDiscretization = (sortedConditions: Differential[], asstr: string[], topn: number, rangeToFit: number) => {
    // take topn + rescale the values to fit range
    const topN = sortedConditions.slice(0, topn)
 ...
 * @param rangeToFit 
 */
export const bottleCapVer2Discretization = (sortedConditions: Differential[], asstr: string[], topn: number, rangeToFit: number) => {
    // take topn + rescale the values to fit range
    const topN = sortedConditions.slice(0, topn)
...
export const getDiscretized = (fn: typeof normalValueDiscretization, sortedConditions: Differential[], asstr: string[] = [], topn: number = 3, rangeToFit: number = 10): { 
    condition: string, 
...
export const getConditionData = (condition: string, conditions: {
    condition: string;
    label: string;
    p: number;
    symptoms: string[];
}[], asstr?: string[]): undefined | null | {
...
export function useElsaLambda () {
    ...
    return { patient, psstr, asstr, getDiscretized, getConditionData, fetchFromElsaLambda }
}
