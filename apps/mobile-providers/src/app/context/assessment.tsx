...
const builderStoreCreator = () => create<AssessmentState>((set, get) => ({
    recommended: [],
    presentingSymptoms: [],
    absentSymptoms: [],
 ...
 */
export function SymptomAssessmentSequenceProvider (props: { children: React.ReactNode }) {
    return (
        <Provider createStore={builderStoreCreator}>
            {props.children}
        </Provider>
    )
}
...
export function useElsaLambda () {
    const patient = usePatientDescription(d => {
        ...
        const p = produce({} as  { age: number, sex: 'male' | 'female'}, df => {
