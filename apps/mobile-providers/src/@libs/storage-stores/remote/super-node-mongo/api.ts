...
export const collection = (snAxios: AxiosInstance) => {
  return <DA extends Store.DocumentAction>(
    ...
    return {
      ...
      queryDoc: async <T>(queryOptions: Store.QueryFilter | undefined) => {
        const res = await snAxios.post(
      ...
      queryDocs: async <T>(queryOptions: Store.QueryFilter | undefined) => {
        const res = await snAxios.post(
          `/col/${name}/query-multiple?${qs.stringify(queryOptions || {})}`,
        );
