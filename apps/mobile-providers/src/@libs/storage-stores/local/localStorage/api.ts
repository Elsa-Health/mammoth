...
export const collection =
  ...
  <DA extends Store.DocumentAction>(
    ...
    const docRefFn = (docId: string) => `${collectionRef}/${genDocRef(docId)}`;
    return {
      create: async opt => {
            ...
            if (Array.isArray($id)) {
              ...
              if ($id !== undefined) {
                ...
                const searchedDoc = docsRefs
                  .map(docId => {
                    const qd = queryDocument<T>(docRefFn(docId));
                    return qd !== null ? qd.data : null;
                  })
                  .filter(d => d !== null)
                  .find(s =>
                ...
                return searchedDoc !== undefined ? searchedDoc : null;
              }
            }
          } else {
            // take the first one
        ...
        if (docsRefs.length > 0) {
          if (qOpts !== undefined) {
            ...
            const content = docsRefs
              ...
              .map(id => {
                const qd = queryDocument<T>(docRefFn(id));
                return qd !== null ? {$id: id, ...qd.data} : null;
              })
              .filter(s => s !== null)
              .filter(s =>
                Object.entries(other)
      ...
      docs: async () => {
        const docsRefs =
          (queryCollection(collectionRef) || {docsRefs: []}).docsRefs || [];
