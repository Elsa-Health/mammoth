...
export const labTests = {
    ids: () => Object.keys(_labTests).sort((a, b) => a.localeCompare(b)) as LabTest[],
    values: () => build<LabTest>(_labTests)
}

const addo = {
    fromId: (id: Medication.Addo) => ({ id, name: medicationAddo[id] }) as { id: Medication.Addo, name: string},
    ids: () => Object.keys(medicationAddo).sort((a, b) => a.localeCompare(b)) as Medication.Addo[],
    values: () => build<Medication.Addo>(medicationAddo)
}
const gs = {
    ids: () => Object.keys(medicationGeneralSales).sort((a, b) => a.localeCompare(b)) as Medication.GS[],
    fromId: (id: Medication.GS) => ({ id, name: medicationGeneralSales[id] }) as { id: Medication.GS, name: string},
