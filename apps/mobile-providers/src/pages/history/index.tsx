...
export function MiniBars<T>({
	conditions,
	decisionKey,
}: {
	conditions: Array<{ condition: T; p: number }>;
	decisionKey: T;
}) {
	return (
...
export function AssessmentRecordItem({
	...
	// constuct proper
	const ageString = React.useMemo(
		() => properAgeString(ar.patient.age),
		[ar]
	);
		...
		>
			<React.Fragment>
				...
				<View style={{ flex: 1 }}>
					<View style={{ marginVertical: 8 }}>
						<Text>
							{ar.patient.sex === "male" ? "Male" : "Female"},{" "}
							{ageString}
						</Text>
						<RevealContent
							show={
								ar.assessmentInfo.presentingSymptoms.length > 0
							}
							style={{ margin: 8 }}
						>
					...
					>
						<View style={{ flex: 1 }}>
							...
							>
								{ar.assessmentInfo.userConditions.map(
									(diagnosis) => {
										return (
											<Text
												key={diagnosis}
											...
											>
												{datafns.conditions.name.fromId(
													diagnosis
												)}
											</Text>
										);
						...
						<RevealContent
							show={
								elsaDiagnosis !== undefined &&
								elsaDiagnosis.length > 0
							}
						...
						>
							...
							<MiniBars
								conditions={(elsaDiagnosis || []).slice(0, 3)}
							/>
						</RevealContent>
					</View>
