...
import { NavigationContext, useNavigation, useNavigationContainerRef } from '@react-navigation/core'

import { buttonStyles } from '../../components/input'
import { CheckIcon, ElsaColorableIcon, ElsaIcon, LibraryIcon, XIcon } from '../../assets/vectors'
import { Layout, Text } from '../../components'
import { SymptomAssessmentSequenceProvider, useSAStore } from '../../app/context/assessment'
import { SymptomInteractionProvider } from '../../app/interactionSymptoms'
import { PatientDescription, PatientDescriptionProvider, usePatientDescription } from './PatientDescriptor/context'
...
import { animated, useSpring, config, useTransition } from '@react-spring/native'
...
export default function MainApp () {
    ...
    const {iconColor, ...styles} = useSpring({
        ...
        shadowColor: "#000",
        shadowOffset: show ? { width: 0, height: 0} : {
            width: 0,
            height: 3,
        }, 
        ...
        <PatientDescriptionProvider>
            <SymptomAssessmentSequenceProvider>
                <SymptomInteractionProvider>
                    <Layout navigation={navigation} style={{paddingHorizontal: 0, paddingVertical: 0 }} backIcon={XIcon} hideHeader>
                        <View style={{ flex: 1, display: 'flex', flexDirection: 'row', position: 'relative' , alignItems: 'center'}} >
                            <View style={[{  }, isTablet ? { flex: 2 }: { flex: 1 }]}>
                            ...
                            <View style={[
                                ...
                                isTablet ? { borderLeftColor: BORDER_COLOR, borderLeftWidth: 1 }: 
                                    (isTablet || (!isTablet && show)) ? { 
                                        position: 'absolute', bottom: 0, shadowColor: "#000",
                                        shadowOffset: { width: 0, height: 1, },
                                        shadowOpacity: 0.22,
                                        shadowRadius: 2.22,
                                        elevation: 3, width: "100%", height: "97%" 
                                    }: { display: 'none' }]
                                }
                            >
                            ...
                            { !isTablet && (
                            <View style={{ position: 'absolute', bottom: -36, right: 24, width: "100%" }}>
                                <View style={{ flex: 1, display: 'flex', flexDirection: 'row', justifyContent: 'flex-end' }}>
                                    ...
                                    <$Pressable 
                                        ...
                                        style={[
                                            ...
                                            { borderRadius: 100, alignSelf: 'flex-start', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 20  },
                                            styles
                                        ]}
                                    >
                                        <$ElsaColorIcon width={30} height={30} style={{ color: iconColor }} />
                                    </$Pressable>
                                </View>
                            </View>
                        )}
                        ...
                        {/* Bottom navigation */} 
                        <View style={{ paddingHorizontal: 24, paddingVertical: 16, borderTopColor: BORDER_COLOR, borderTopWidth: 1, backgroundColor: "#FFF", zIndex: 30 }}>
 ...
 */
const convertData = (data: PatientDescription, presentingSymptoms: Assessment['presentingSymptoms'], absentSymptoms: Assessment['absentSymptoms']): PatientAssessmentRecord => {
    const { assessment, dietaryHistory, medicalHistory, patientIntake, elsa } = data

    if (patientIntake === undefined) {
        throw new Error("Patient information is undefined. Unable to convert to assessment record")
    }
...
function BottomNavigationComponent () {
    ...
    const [presentingSymptoms, absentSypmtoms] = useSAStore(s => [s.presentingSymptoms, s.absentSymptoms])
    const selectedConditions = usePatientDescription(s => s.assessment.conditions)
    ...
    const completeAssessment = React.useCallback(() => {
        ...
        addPatientRecord(convertData(pediatricsData, presentingSymptoms, absentSypmtoms))
        navigation.dispatch(StackActions.replace('app.history'))
    }, [pediatricsData, addPatientRecord, navigation, presentingSymptoms, absentSypmtoms])

    return (
        <View style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', backgroundColor: "#FFF" }}>
            <View style={{ flex: 1 }}>
                <View style={{ display: 'flex', flexDirection: 'row', justifyContent: "flex-start"}}>
                    ...
                    >
                        <XIcon />
                        <Text font="bold" style={{ paddingHorizontal: 8 }}>{"Discard"}</Text>
                    </Pressable>
                    {conditionsMeet && (
                        <Pressable
                            android_ripple={{ borderless: true, radius: 16 }} 
                        ...
                        >
                            <CheckIcon />
                            <Text font="bold" style={{ paddingHorizontal: 8 }}>{isTablet ? "Complete Assessment": "Complete"}</Text>
                        </Pressable>
                    )}
                </View>
            </View>
