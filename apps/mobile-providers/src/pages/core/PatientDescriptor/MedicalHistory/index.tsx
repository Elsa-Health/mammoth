...
const standardizeData = (data: MedicationHistoryForm): MedicationHistory => {
	...
	return {
		previous,
		timeHospitalized: timeHospitalized
			? parseInt(timeHospitalized)
			: undefined,
		bloodTransfusion,
	};
};
...
export default function MedicalHistory() {
	...
	const [vals, setVals] = React.useState<MedicationHistoryForm>(() =>
		medicalHistory !== undefined
			? revertData(medicalHistory)
			: DEFAULT_MEDICAL_HISTORY_STATE
	);

	const setMedicalHistory = usePatientDescription(
		(s) => (medicalHistory: MedicationHistory) =>
			s.setData({ medicalHistory })
	);
		...
		>
			...
			>
				...
				<RevealContent show={vals.previous} style={{ marginBottom: 8 }}>
					...
					<View>
						<TextInput
							placeholder="Years"
							value={vals.timeHospitalized}
							onChangeText={(text) =>
								setVals((k) =>
									produce(k, (df) => {
										df["timeHospitalized"] = text;
									})
								)
							}
				...
				<View style={{ marginBottom: 8 }}>
					...
					>
						<SelectableChip
							text="Yes"
							selected={vals?.bloodTransfusion}
							onPress={() =>
								setVals((k) =>
									produce(k, (df) => {
										df["bloodTransfusion"] = true;
									})
								)
							}
						/>
			...
			<View style={{ paddingHorizontal: 24, marginVertical: 6 }}>
				<Text style={{ fontSize: 14, color: "#777", marginBottom: 6 }}>
					Clicking next would save the latest medical history
