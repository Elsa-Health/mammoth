...
import { CheckIcon, MenuIcon, NextIcon, PencilAltIcon, XIcon } from '../../../assets/vectors'
...
function ConditionViewSection ({ mainNavigator, conditions }: { mainNavigator: any, conditions: any[] }) {
...
export default function SummaryView ({ mainNavigator }: { route: any, mainNavigator: any }) {
    ...
    const setElsaData = usePatientDescription(s => (conditions: Differential[], computed: any[]) => s.setData({ elsa: { conditions, computed }}))
    ...
    return (
        <Layout hideHeader style={{ padding: 0, flex: 1 }}>
            <ScrollView style={{ marginVertical: 18, flex: 1 }} alwaysBounceVertical showsVerticalScrollIndicator>
                {/* Header */}
                ...
                <View style={{ marginVertical: 10, paddingHorizontal: 24 }}>
                    <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'  }}>
                        <Text font="bold" style={{ fontSize: 17 }}>Complaints</Text>
                        <Pressable
                            ...
                            symptoms.length > 0 ? (
                                <View style={{ flex: 1, paddingHorizontal: 24 }}>
                                    <View style={{ marginVertical: 10, marginTop: 20 }}>
                                        ...
                                        <View>
                                            <ConditionViewSection mainNavigator={mainNavigator} conditions={conditions.length === 0 ? [] : getDiscretizedConditions(conditions, asstr)} />
                                        </View>
                                    </View>
                                </View>
                            ) : null
                ...
                {/* Patient */}
                <View style={{ marginVertical: 10, paddingHorizontal: 24, paddingVertical: 16, borderTopWidth: 1, borderTopColor: "#4BB8E9" }}>
                    <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'  }}>
                        <Text font="bold" style={{ fontSize: 17 }}>Patient Summary</Text>
                        {
                    ...
                    {
                        patientIntakeData !== undefined && (patientIntakeData.allergies.length > 0 || patientIntakeData.chronicIllnesses.length > 0 || patientIntakeData.pregnant) ? (
                            // Shows the patient related to the Information
                            <View>
                                {/* For allergies */}
                                <RevealContent show={patientIntakeData.allergies.length > 0} style={{ marginBottom: 10 }}>
                                    <Text>Allergies:</Text>
                                    {
                                        patientIntakeData.allergies.map((m, ix) => (
                                            <View key={ix} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start' }}>
                                                <View><Text>{ix + 1}.</Text></View>
                                                {/* <View style={{ width: 8, height: 8, backgroundColor: theme.color.primary.dark, borderRadius: 8 }} /> */}
                                                <Text style={{ marginLeft: 10 }} >{m}</Text>
                                            </View>
                                        ))
                                    }
                                </RevealContent>
                                ...
                                <RevealContent show={patientIntakeData.chronicIllnesses.length > 0} style={{ marginBottom: 10 }}>
                                    <Text>Chronic Illnesses:</Text>
                                    {
                                        patientIntakeData.chronicIllnesses.map((m, ix) => (
                                            <View key={ix} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start' }}>
                                                <View><Text>{ix + 1}.</Text></View>
                                                {/* <View style={{ width: 8, height: 8, backgroundColor: theme.color.primary.dark, borderRadius: 8 }} /> */}
                                                <Text style={{ marginLeft: 10 }} >{m}</Text>
                            ...
                            <View>
                                <Text italic style={{ textAlign: 'center', color: "#555"  }}>
                                    Data associated with this patient is yet available. It will show up here once it is available.
                                </Text>
                            </View>
                        )
                    }
